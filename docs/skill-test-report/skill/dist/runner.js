...
function runExecuteMode(ctx) {
    ...
    try {
        ...
        if (exitCode !== 0 && stdout.trim() === "" && stderr.trim() !== "") {
            throw new TestReportError("TEST_COMMAND_FAILED", `测试命令无法运行：${cmd}`, { diagnostic: `exit=${exitCode}\nstderr(前300字)：${stderr.slice(0, 300)}` });
        }
    }
    ...
    if (parseSrc.trim() === "") {
        throw new TestReportError("RESULT_FILE_EMPTY", "执行模式未能收集到测试结果（stdout 为空且未发现结果文件）。", { diagnostic: `exit=${exitCode}, stderr(前300字)=${stderr.slice(0, 300)}` });
    }
    return parseContent(parseSrc, { rawResultFilePath: rawPath, sanitize: defaultSanitize });
}
function looksUserSpecified(cmd) {
    // 用户显式指定的命令通常含空格参数或特定标志
    return cmd.includes("--") || cmd.includes(" ") && !cmd.startsWith("npx vitest run") && !cmd.startsWith("npx jest");
}
function wrapForJUnit(cmd) {
    if (/vitest/.test(cmd))
        return `${cmd} --reporter=junit --outputFile=test-results.xml`;
    if (/jest/.test(cmd))
        return `${cmd} --json --outputFile=test-results.json`;
    // 通用：假设目标项目已配置报告器，原样执行
    return cmd;
}
// === 报告组装 ===
function assembleReport(ctx, pr, config) {
    const now = Date.now();
...
function renderHtmlFallback(report) {
    const md = renderMarkdown(report, { ...{}, outputFormat: "markdown" });
