...
function parseArgs(argv: string[]): { cwd: string; config: Partial<SkillConfig>; resultFile?: string; testCommand?: string } {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case "--test-command":
        ...
        break;
      case "--output-format":
        config.outputFormat = (argv[++i] as SkillConfig["outputFormat"]) ?? "markdown";
        break;
      case "--output-path":
        config.outputPath = argv[++i];
        break;
      case "--coverage":
        config.coverage = (argv[++i] as SkillConfig["coverage"]) ?? "auto";
        break;
      case "--fail-threshold":
...
function main(): void {
  ...
  const merged: SkillConfig = { ...DEFAULT_CONFIG, ...config } as SkillConfig;
  try {
    ...
    if (e instanceof TestReportError) {
      ...
      if (e.diagnostic) console.error(`  诊断：${e.diagnostic}`);
      process.exit(1);
    }
    throw e;
  }
