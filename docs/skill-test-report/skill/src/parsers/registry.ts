// Test Report Skill · 解析器注册表 (registry.ts) — NFR5
...
/** 获取已注册插件列表（只读快照） */
export function listParsers(): readonly ParserPlugin[] {
  return registry.slice();
}

/** 嗅探：依据内容自动选择首个可解析的插件 */
export function detectByContent(content: string): ParserPlugin | undefined {
...
/** 按框架 id 取插件 */
export function getParser(id: FrameworkId): ParserPlugin | undefined {
  return registry.find((p) => p.id === id);
}

/** 统一解析入口：内容嗅探 → parse，失败抛明确错误 */
export function parseContent(content: string, opts?: ParseOptions): ParseResult {
  ...
  if (!plugin) {
    throw new TestReportError(
      ...
      "无法识别结果文件格式：未匹配任何已注册解析器。",
      { diagnostic: summarizeForDiagnostic(trimmed) },
    );
  }
  return plugin.parse(trimmed, opts);
}

function summarizeForDiagnostic(content: string): string {
