// Test Report Skill · 解析层共享工具 (shared.ts)
import type { TestCase, TestCaseError, TestFileGroup, TestSummary } from "../types.js";
...
/** 截断堆栈至可读长度（保留前若干关键行） */
export function truncateStack(stack: string, maxLines = 12, maxChars = 800): string {
...
export function buildError(rawMessage: string, rawStack: string | undefined, sanitize: (s: string) => string): TestCaseError {
  const message = sanitize(rawMessage ?? "") || "（未获取错误信息）";
  const err: TestCaseError = { message };
...
export function buildTestCaseId(filePath: string, name: string, index: number): string {
  const fp = (filePath ?? "").trim();
  const nm = (name ?? "").trim();
  if (fp && nm) return `${fp}::${nm}`;
  if (nm) return nm;
  return `case-${index}`;
}

/** 由文件分组汇总 TestSummary */
export function summarizeFromGroups(fileGroups: TestFileGroup[], totalDurationMs: number): TestSummary {
  let total = 0,
    passed = 0,
    failed = 0,
    skipped = 0,
...
export function durationMsFromSeconds(s: number | undefined): number {
  if (s == null || Number.isNaN(s)) return 0;
  return Math.round(s * 1000);
}

export function durationMsFromMs(ms: number | undefined): number {
