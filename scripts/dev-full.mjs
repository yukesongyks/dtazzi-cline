...
const webUiPort = await findPort(8080, new Set([runtimePort]));
const requestedDevFullArgs = process.argv.slice(2);
const withShutdownCleanupFlag = "--with-shutdown-cleanup";
// Filter out --port and --host since they're managed by this script
const filteredArgs = [];
for (let i = 0; i < requestedDevFullArgs.length; i++) {
	const arg = requestedDevFullArgs[i];
...
const requestedRuntimeArgs = filteredArgs.filter((arg) => arg !== withShutdownCleanupFlag);
...
const env = {
	...
	KANBAN_DEV_HOSTS: "dtagenticflow.alipay.net",
	...process.env,
