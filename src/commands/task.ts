...
import { runtimeAgentIdSchema, runtimeClineReasoningEffortSchema, runtimeTaskWorkspaceModeSchema } from "../core/api-contract";
import { buildKanbanRuntimeUrl, getKanbanRuntimeOrigin, getRuntimeFetch } from "../core/runtime-endpoint";
import { assertMemoryAvailable } from "../core/system-memory";
import {
	addTaskDependency,
	addTaskToColumn,
...
function parseTaskWorkspaceMode(value: string | undefined): RuntimeTaskWorkspaceMode | undefined {
	if (value === undefined) {
		return undefined;
	}
	const result = runtimeTaskWorkspaceModeSchema.safeParse(value);
	if (result.success) {
		return result.data;
	}
	throw new Error('Invalid workspace mode. Expected one of: worktree, project_root.');
}
...
async function createTask(input: {
	...
	return {
		...
		task: {
			...
			workspaceMode: created.workspaceMode ?? "worktree",
...
async function updateTaskCommand(input: {
	...
	const updated = await updateRuntimeWorkspaceState(runtimeClient, workspaceRepoPath, (runtimeState) => {
		...
		const updatedTask = updateTask(runtimeState.board, input.taskId, {
			...
			agentId: input.agentId,
			workspaceMode: input.workspaceMode ?? taskRecord.task.workspaceMode ?? "worktree",
...
async function startTask(input: { cwd: string; taskId: string; projectPath?: string }): Promise<JsonRecord> {
	...
	if (shouldStartSession) {
		if ((task.workspaceMode ?? "worktree") === "worktree") {
...
export function registerTaskCommand(program: Command): void {
	...
	task
		...
		.option("--workspace-mode <mode>", "Task workspace mode: worktree | project_root.", parseTaskWorkspaceMode)
	...
	task
		...
		.option("--workspace-mode <mode>", "Replacement workspace mode: worktree | project_root.", parseTaskWorkspaceMode)
		.option("--start-in-plan-mode [value]", "Set plan mode (true|false). Flag-only implies true.")
		.option("--auto-review-enabled [value]", "Enable auto-review behavior (true|false). Flag-only implies true.")
		.option("--auto-review-mode <mode>", "Auto-review mode: commit | pr.", parseAutoReviewMode)
		.option(
