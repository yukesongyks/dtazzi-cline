...
// Session saves merge onto the persisted record set instead of replacing it.
// When the same task exists in both maps, the newer updatedAt wins; incoming
// sessions win ties so live terminal snapshots can refresh persisted history.
...
export async function saveWorkspaceState(
	...
	return await lockedFileSystem.withLock(getWorkspaceDirectoryLockRequest(context.workspaceId), async () => {
		...
		let currentSessions: Record<string, RuntimeTaskSessionSummary> = {};
		try {
			currentSessions = await readWorkspaceSessions(context.workspaceId);
		} catch {
			// Preserve the pre-existing self-healing behavior for malformed
			// sessions.json files by overwriting them on the next successful save.
			currentSessions = {};
		}
