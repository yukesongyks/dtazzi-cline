...
import { buildExecutablePath } from "./command-discovery";
...
const MAX_STARTUP_INPUT_BUFFER_CHARS = 16_384;
...
interface ActiveProcessState {
	...
	startupInputBuffer: string | null;
...
function buildTerminalEnvironment(
	...
	return {
		...
		PATH: buildExecutablePath(Object.assign({}, ...sources).PATH ?? process.env.PATH),
		COLORTERM: "truecolor",
		TERM: "xterm-256color",
		TERM_PROGRAM: "kanban",
	};
...
function hasKimiCodeStartupInputReady(text: string): boolean {
	// kimi-code (TS rewrite) renders a different TUI than kimi-cli (Python).
	// Wait for the footer status line containing `context: ...%` AFTER the
	// boxed `>` prompt — this only appears once the input box is mounted and
	// accepting keystrokes.
	const stripped = stripAnsi(text);
	return /context:\s*\d/u.test(stripped) && /(?:^|[\n\r])\s*[│┃▎▏▌]?\s*>\s/u.test(stripped);
}

function isKimiFamily(agentId: RuntimeTaskSessionSummary["agentId"]): boolean {
	return agentId === "kimi" || agentId === "kimi-code";
}

function isKimiFamilyStartupInputReady(agentId: RuntimeTaskSessionSummary["agentId"], text: string): boolean {
	if (agentId === "kimi-code") {
		return hasKimiCodeStartupInputReady(text);
	}
	return hasKimiStartupInputReady(text);
}
...
export class TerminalSessionManager implements TerminalSessionService {
	...
	private trySendDeferredStartupInput(taskId: string, agentId: RuntimeTaskSessionSummary["agentId"]): boolean {
		const entry = this.entries.get(taskId);
		const active = entry?.active;
		if (!entry || !active || entry.summary.agentId !== agentId) {
			return false;
		}
	...
	async startTaskSession(request: StartTaskSessionRequest): Promise<RuntimeTaskSessionSummary> {
		...
		try {
			session = PtySession.spawn({
				...
				onData: (chunk) => {
					...
					const needsDecodedOutput =
						...
						(entry.active.deferredStartupInput !== null &&
							(entry.summary.agentId === "codex" || isKimiFamily(entry.summary.agentId))) ||
					...
					// Some TUIs drop input sent before the prompt is ready, so defer startup input
					// until we observe their prompt/header.
					if (
					...
					if (
						isKimiFamily(entry.summary.agentId) &&
						entry.active.deferredStartupInput !== null &&
						...
						if (entry.active.startupInputBuffer.length > MAX_STARTUP_INPUT_BUFFER_CHARS) {
							entry.active.startupInputBuffer = entry.active.startupInputBuffer.slice(
								-MAX_STARTUP_INPUT_BUFFER_CHARS,
							);
						}
						if (isKimiFamilyStartupInputReady(entry.summary.agentId, entry.active.startupInputBuffer)) {
							const kimiAgentId = entry.summary.agentId;
							entry.active.startupInputBuffer = null;
							setTimeout(() => {
								this.trySendDeferredStartupInput(request.taskId, kimiAgentId);
							}, 750);
						}
					}
				...
				onExit: (event) => {
					...
					const wasAutoRestartSuppressed = this.consumeAutoRestartSuppression(currentEntry);
					const shouldAutoRestart = this.shouldAutoRestart(currentEntry, wasAutoRestartSuppressed);
					...
					this.emitSummary(summary);
					if (currentEntry.deleteOnExit && !shouldAutoRestart) {
						this.entries.delete(request.taskId);
					}
		...
		const active: ActiveProcessState = {
			...
			startupInputBuffer: isKimiFamily(request.agentId) && launch.deferredStartupInput ? "" : null,
		...
		if (request.agentId !== "codex" && !isKimiFamily(request.agentId) && active.deferredStartupInput !== null) {
			const deferredInput = active.deferredStartupInput;
			active.deferredStartupInput = null;
			active.session.write(deferredInput);
		}
	...
	private consumeAutoRestartSuppression(entry: SessionEntry): boolean {
		const wasSuppressed = entry.suppressAutoRestartOnExit;
		entry.suppressAutoRestartOnExit = false;
		return wasSuppressed;
	}

	private shouldAutoRestart(entry: SessionEntry, wasSuppressed = false): boolean {
		if (wasSuppressed) {
			return false;
		}
		...
		if (entry.summary.state === "awaiting_review" && entry.summary.reviewReason === "exit") {
			return false;
		}
