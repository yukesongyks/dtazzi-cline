...
import {
	...
	runtimeAntcodeIssuesResponseSchema,
...
import { assertMemoryAvailable, checkSystemMemory } from "../core/system-memory";
...
export const runtimeAppRouter = t.router({
	runtime: t.router({
		...
		getSystemMemory: t.procedure.output(runtimeSystemMemoryResponseSchema).query(async (): Promise<RuntimeSystemMemoryResponse> => {
			const status = await checkSystemMemory();
			return { totalMemory: status.totalMemory, freeMemory: status.freeMemory, usagePercent: status.usagePercent };
		}),
		...
		getAntcodeIssues: workspaceProcedure
			.input(runtimeAntcodeIssuesRequestSchema)
			.output(runtimeAntcodeIssuesResponseSchema)
			.query(async ({ ctx, input }) => {
				return await ctx.runtimeApi.getAntcodeIssues(ctx.workspaceScope, input);
			}),
		getAntcodeIssueDetail: workspaceProcedure
			.input(runtimeAntcodeIssueDetailRequestSchema)
	...
	workspace: t.router({
		...
		saveState: workspaceProcedure
			...
			.mutation(async ({ ctx, input }) => {
				const memStatus = await checkSystemMemory();
				if (!memStatus.ok) {
					const currentState = await ctx.workspaceApi.loadState(ctx.workspaceScope);
					const currentCardCount = currentState.board.columns.reduce((sum, col) => sum + col.cards.length, 0);
					const incomingCardCount = input.board.columns.reduce((sum, col) => sum + col.cards.length, 0);
					if (incomingCardCount > currentCardCount) {
						throw new Error(
							`System memory is critically low (${memStatus.usedGB}/${memStatus.totalGB} GB used, ${memStatus.usagePercent}% usage). ` +
