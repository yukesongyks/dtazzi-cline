...
export class TestReportError extends Error {
  ...
  constructor(code: SkillErrorCode, message: string, opts?: { diagnostic?: string; cause?: unknown }) {
    super(message);
  ...
  toSkillError(): SkillError {
    return { code: this.code, message: this.message, diagnostic: this.diagnostic, cause: this.cause };
  }
}
