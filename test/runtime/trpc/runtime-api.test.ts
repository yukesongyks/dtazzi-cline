...
describe("createRuntimeApi startTaskSession", () => {
	...
	it("starts native agents with the selected configured instance command", async () => {
		...
		runtimeConfigState.selectedAgentInstanceId = "claude-kimi";
		runtimeConfigState.configuredAgents = [
			{
				id: "claude-theta",
				type: "claude",
	...
	it("starts project-root task sessions in the workspace root without resolving a task worktree", async () => {
		const terminalManager = {
			startTaskSession: vi.fn(async () => createSummary()),
			applyTurnCheckpoint: vi.fn(),
		};
