...
export default function App(): ReactElement {
	...
	const {
		...
		memoryUsagePercent: systemMemory?.usagePercent,
	});
	...
	const inlineTaskEditor = editingTaskId ? (
		<TaskInlineCreateCard
			...
			onAgentInstanceIdChange={setEditTaskAgentInstanceId}
			clineSettings={editTaskClineSettings}
			onClineSettingsChange={setEditTaskClineSettings}
					scheduledStartTime={editTaskScheduledStartTime}
					onScheduledStartTimeChange={setEditTaskScheduledStartTime}
					agentDefinitions={runtimeProjectConfig?.agents ?? []}
		...
		<LayoutCustomizationsProvider onResetBottomTerminalLayoutCustomizations={resetBottomTerminalLayoutCustomizations}>
			<div className="flex h-[100svh] min-w-0 overflow-hidden">
				...
				<TaskCreateDialog
					...
					onScheduledStartTimeChange={setNewTaskScheduledStartTime}
					agentDefinitions={runtimeProjectConfig?.agents ?? []}
					configuredAgents={runtimeProjectConfig?.configuredAgents ?? []}
					defaultAgentId={runtimeProjectConfig?.selectedAgentId ?? null}
					defaultAgentInstanceId={runtimeProjectConfig?.selectedAgentInstanceId ?? null}
