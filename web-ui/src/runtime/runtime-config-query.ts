...
export async function fetchAntcodeIssues(
	workspaceId: string,
	options?: { search?: string; perPage?: number },
): Promise<RuntimeAntcodeIssuesResponse> {
	const trpcClient = getRuntimeTrpcClient(workspaceId);
	return await trpcClient.runtime.getAntcodeIssues.query(options ?? {});
}

export async function fetchAntcodeIssueDetail(
	workspaceId: string,
	iid: number,
): Promise<RuntimeAntcodeIssueDetailResponse> {
	const trpcClient = getRuntimeTrpcClient(workspaceId);
	return await trpcClient.runtime.getAntcodeIssueDetail.query({ iid });
}
