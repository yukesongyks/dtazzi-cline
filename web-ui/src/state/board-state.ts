...
function normalizeCard(rawCard: unknown): BoardCard | null {
	...
	return {
		...
		...(typeof card.agentInstanceId === "string" && card.agentInstanceId
			? { agentInstanceId: card.agentInstanceId as RuntimeAgentInstanceId }
			: {}),
		...(clineSettings !== undefined ? { clineSettings } : {}),
		workspaceMode: card.workspaceMode === "project_root" ? "project_root" : "worktree",
	...
	const columns = board.columns.map((column) => {
		...
		const cards = column.cards.map((card) => {
			...
			return {
				...
				agentInstanceId: draft.agentInstanceId,
				clineSettings: draft.clineSettings,
				workspaceMode: draft.workspaceMode ?? card.workspaceMode ?? "worktree",
				baseRef,
				updatedAt: Date.now(),
			};
		});
...
export function updateTaskTitle(
	...
	return updateTask(board, taskId, {
		...
		agentInstanceId: selection.card.agentInstanceId,
